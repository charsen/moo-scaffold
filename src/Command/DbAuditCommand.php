...
class DbAuditCommand extends Command
{
    ...
    public function handle(SchemaDbAuditor $auditor): int
    {
        ...
        if (! $auditor->isSupported()) {
            $this->console()->warn('无法对账：当前默认连接不是 mysql，或数据库不可达。');
            $this->console()->line('<fg=gray>（本命令依赖 mysql information_schema 反查实际表结构）</>');
        ...
        foreach ($schemas as $schema) {
            ...
            if ($rows === []) {
                $this->console()->line("  <fg=green>✓</> {$schema}  <fg=gray>yaml 与 DB 一致</>");
            ...
            $n = count($rows);
            $this->console()->line("  <fg=yellow>⚠ {$schema}</>  <fg=gray>（{$n} 处漂移）</>");
            $this->printRows($rows);
        }

        $this->console()->line('');
        if ($totalDrift === 0) {
            $this->console()->success('全部 ' . count($schemas) . ' 个 schema 的 yaml 与实际 DB 一致，无漂移。');

            return self::SUCCESS;
        }

        $this->console()->warn("发现 {$totalDrift} 处 yaml↔DB 漂移，涉及 {$dirtySchemas} 个 schema。");
        $this->console()->line('<fg=gray>  请按 DB 现状修正对应 yaml（designer 改或手改），改完可重跑本命令复核；</>');
        $this->console()->line('<fg=gray>  若 baseline 也需同步，再跑 moo:snapshot:init --schema=X --force。</>');
    ...
    private function printRows(array $rows): void
    {
        ...
        foreach ($byTable as $table => $items) {
            $this->console()->line("      <fg=gray>{$table}</>");
            foreach ($items as $r) {
