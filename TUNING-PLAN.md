...
| 拆大类·SchemaLoader（2026-09-20 第 6 项：`loadTableFull` 族） | **同一判据的第三个实例，也是本批唯一「三层全满足」的切片**。判据三层：① 行数多不多（**不是**判据）；② 有没有「单入口可达的族」；③ **族内有没有零状态子集**。本族三层全中 —— 只被 `loadTableFull` 一个 public 入口可达、族外接触面 0、**整族 0 处实例状态**（4 个方法只吃入参 + `ColumnTypeGroups::` 静态常量）：`shape`（原 `shapeField`，99 行块）+ `computeSizeClass` / `computeDefaultClass` / `computeDefaultTitle`。**反向判据（本轮新立）**：零状态子集**占比过高**时摘出去等于把整个类搬家 —— `SchemaDiffService::diff` 16/17 = **97%**、摘完只剩一个壳持 2 个状态，那是加间接层，**明确否决**；**闸门**是「外迁后『新家』与『旧家剩余』各自成为说得清的职责」，本项两边分别是**纯函数 shaper** 与 **yaml I/O**，成立。用户已批准外迁 `Designer\FieldShaper`（**不留转发**）。判据与实测、以及「族级读数是聚合读数、会掩盖族内零状态子集」（`CreateApiGenerator` 族级持 14 处状态、但族内 17 方法 / 286 行块零状态）见 `NOTES.md` 同日两条 + `family_scan.py` §4b。 |
| 拆大类·ScaffoldController（2026-09-20 第 7 项：publish-history 族） | **同一判据的第四个实例，也是「内聚但要注入依赖」型的第一个样板** —— 前三个实例（`ApiParameterFormatter` / `SchemaPayloadMerger` / `FieldShaper`）搬的都是**零状态**族，本项是**首个非零状态**外迁，因此这是「整族零状态」队列用尽之后那条路的可行性证明。判据三层：① 单入口可达的族 —— 族外只从 `getDashboardStats` 经 3 个入口进来，**跨文件调用者 0**；② 族内**内聚** —— 全部围绕「一份历史 yaml → 一条首页记录 → 一个 tab 的一页」；③ 拆完两边各自成立 —— 新家 = 发布历史读取，旧家 = 首页/路由/字典页渲染。**⚠ 判据第三层这时才真正发力**：`ScaffoldController::index` 的 11/294 被同一条判据**否决**（横跨 5 个职责 = 杂物抽屉），而 publish-history 族 7 方法 / 285 行块**整体是一个职责** ⇒ 同一层判据给出相反结论，差别就在「内聚」。**反向判据仍然适用**：本族占控制器 795 行的 36%（第 6 项那条「占比过高 = 只是搬家」的闸门未触发）。**关键技法（可复用）**：让注入的属性名与旧宿主逐字相同（`$filesystem` / `$utility`，正是基类 `Controller` 的属性名）⇒ 方法体一个字都不用改，机械替换只剩 3 个入口 `private` → `public` ⇒ **有状态族同样拿到字节级零差异证明**（285 行零差异 + 方法集合差集 21 → 14）。用户已批准外迁 `Support\PublishHistoryService`（**不留转发**）。判据、两种机器证明、44 处 mutation（预检 17 + 搬后 17 + 结构 10、漏网 0）见 `NOTES.md` 同日三条。 |
| 拆大类·AuditFormContractCommand（2026-09-20 第 8 / 9 项：扫描目标推导族 + 豁免标记族） | **同一份命令连续拆两次，两族都是「零状态 ⇒ 全静态薄类」**，也是「内聚 ≠ 名字像」的一条实证。**第 8 项**外迁 `Support\ControllerScanTarget`（3 方法 / 73 行：`resolveRoot` / `controllerNamespace` / `requestNamespace`）；**第 9 项**外迁 `Support\FormContractMarkers`（3 方法 / 74 行 + 私有常量 `WAIVED_MARKER_PATTERN`，该常量被 `self::` 引用 ⇒ **必须跟着搬**）。整份命令 **757 → 592 行、方法 18 → 12**（第 8 项 −83 行，第 9 项 −82 行）。**⚠ 本行同时订正一条既有侦察记录**：早前记的「命名空间解析 4 方法（含 `inspectController`）」**被推翻** —— `inspectController` 是执行驱动、还调族外 `inspectFormPath`，**与解析不是一个职责**；刻意不搬的「同族」是 `resolveNamespace` / `resolveControllerFiles`（要 `$this->option()` / `$this->console()`）。**归簇要按职责，不是按名字像不像。** **⚠ 新增一条「改前先绿」的反向陷阱（第 4 种）**：既有结构锚点会**随外迁失效** —— `AuditFormContractCommandTest` 的 10 项私有方法清单含 `dropForgotten` / `staleWaivedMarkers`，**必须在同一次改动里删掉**否则搬完立刻红；应对 = 外迁前先 `grep -rn <族内方法名>` 把「别处的结构锚点」列进出迁清单。**两种机器证明**：方法（+常量）集合差集 + 字节级重建比对（第 9 项需**两级重建**：`HEAD → 施加第 8 项变换 = pre-N → 施加第 9 项变换`，且切片坐标必须取自相邻前态）。**验证**：全量 pest 第 8 项后 1 / 3 / 1312 / 5485、第 9 项后 1 / 3 / 1320 / 5529（唯一红仍是既有基线 `ConfigControllerTest:305`；⚠ 第 9 项增量 **+44 ≠ 新测试 48**，差额 −4 来自上面那条锚点清单同步删 2 项 ⇒ **增量必须逐项对上**）；**mutation 合计 60 处、漏网 0**（第 8 项 9+9+10；第 9 项 10+10+12，含「常量没跟着搬」「常量没删干净」两条常量专属变异 + 每项各一次半迁移实验）。**e2e 可跳过**（六·五·零：无 `public/`、无 `*.blade.php`）；**跨仓影响 0**。见 `NOTES.md` 同日四条 + `family_scan.py`。 |
