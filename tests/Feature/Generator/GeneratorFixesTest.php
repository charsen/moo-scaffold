...
it('⑥ 整型判断统一走 ColumnTypeGroups;TS 的 number 分支用 INT_NO_BIGINT;UNSIGNED_DEFAULT 故意窄', function () {
    ...
    // 收口到 ColumnTypeGroups::INT。本条原先把"字面长什么样"当锚点;收口后改断"接线到单一来源",
    // 成员是否含 smallint/mediumint 由 FieldTypesTest 的成员锁负责(两层各管一段,别互相替代)。
    expect(substr_count($model, 'ColumnTypeGroups::INT'))->toBe(3);
    // decimal 与 float/double 同等待遇
    expect($model)->toContain('ColumnTypeGroups::FLOAT');

    // TS 的 number 分支必须是 INT_NO_BIGINT —— bigint 仍走独立 'bigint | string'
    // (JS number 装不下 64 位),不是漏写。
    expect($ts)->toContain('ColumnTypeGroups::INT_NO_BIGINT');
    ...
    expect($fresh)->toContain('ColumnTypeGroups::UNSIGNED_DEFAULT');
