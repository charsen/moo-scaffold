...
it('moo:model:Model/Filter/Trait/Enum 全落包 src/Models(平铺),命名空间用包根', function () {
    ...
    expect(is_file("{$base}/BaseFilter.php"))->toBeFalse();
    expect(is_file("{$base}/Traits/UsingSnowFlakePrimaryKey.php"))->toBeFalse();

    $filter = file_get_contents("{$base}/Filters/PkgxItemFilter.php");
    expect($filter)->toContain('use Mooeen\\Scaffold\\Foundation\\BaseFilter;');
