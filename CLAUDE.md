...
1. 默认 effort=high，简单活降到 medium/low，真难的再上 xhigh，别无脑拉 max。
2. 交代目标和边界，让模型自己规划路径，别把每一步写死；旧模型那套逐条「你必须/CRITICAL」的叮嘱能省则省。上面「协同开发原则与边界」是红线，不在此列——红线要硬，路径要松。
3. 长活走异步：交代清楚就去忙别的，回头收结果；跑十几分钟很正常。
