...
YAML 驱动的一条生成流水线。**生成类命令 dev-only**（非 local 环境直接退出，防线即 `config('scaffold.only_in_local')`）；以下运维 / 协同类命令**例外，任何环境可跑**（scaffold 自有的显式 `requiresLocalEnvironment = false`）：`moo:fresh`（缓存被 git 忽略，生产需重建）、`moo:account:add`（首账号引导）、`moo:db:audit`（只读对账，含 prod 核对）、`moo:audit:form-contract`（只读表单契约审计，默认只报用户可见违规）、`moo:audit:resource-keys`（只读整数键映射体检，抽样核对该列真实数据）、`moo:audit:former-types`（只读跨仓体检：SPA `former/config.ts` 控件类型清单 ↔ 后端 `FormWidgetTypes::FORMER`，不一致退出码 1）、`moo:composer:docs`（只读体检：宿主私包清单文档 ↔ 三份 manifest）、`moo:scaffold:merge-yaml`（多端同步脚本调用）；另有 `moo:cloud:push` / `moo:cloud:mcp` / `moo:monitor:migrate`（生产机推云 / AI 接入 / 旧版迁移）由依赖包 [moo-monitor-laravel](https://github.com/charsen/moo-monitor-laravel) 提供，不受 `only_in_local` 限制。
...
| `moo:audit:form-contract` | 表单契约 | 只读断言 create/edit 表单实际渲染的控件都被 Store / Update Request 收下（默认只报用户可见违规；`// @moo-waived <字段>: <原因>` 豁免） |
| `moo:audit:resource-keys` | 整数键映射 | 只读体检「Resource 透出的 json 列」里的整数键映射（出参会被 `removeMissingValues()` 抹键）；抽样真实数据判定，`--fail-on-danger` 可当闸门 |
| `moo:audit:former-types` | 控件类型清单 | 只读跨仓断言 SPA `former/config.ts` 的 `elComponents` 注册表 ↔ 后端 `FormWidgetTypes::FORMER` 是同一集合（`--spa=` 必填）；不一致退出码 1，读取/解析失败退出码 2 |
